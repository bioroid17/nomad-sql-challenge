...
FROM
  books
WHERE
  average_rating BETWEEN 4.5 AND 5.0
LIMIT
  100;
...
FROM
  books
WHERE
  published_date BETWEEN 2015 AND 2023;
...
WITH
  ...
  max_rating_cte AS (
    SELECT
      max_avg_rating
    FROM
      prolific_authors_cte
    WHERE
      prolific_authors_cte.author = main_books.author
  )
SELECT
  title,
  author,
  average_rating AS max_avg_rating,
  (
    SELECT
      max_avg_rating
    FROM
...
FROM
  books AS main_books
WHERE
  main_books.average_rating = (
    SELECT
      max_avg_rating
    FROM
      best_book_cte
    WHERE
      main_books.published_date BETWEEN (
        SELECT
          MIN(published_date)
        FROM
          best_book_cte
...
WITH
  least_ten_books_cte AS (
    SELECT
      author,
      COUNT(*) book_count,
      AVG(average_rating) avg_average_rating
    FROM
      books AS ltbc_books
    GROUP BY
      author
...
WHERE
  main_books.pages < (
    ...
    FROM
      under_avg_page_cte
    WHERE
      main_books.published_date = published_date
  )
  AND main_books.average_rating > (
    SELECT
      avg_average_rating
    FROM
      short_book_avg_rating_cte
  )
  AND main_books.author IN (
    SELECT
      author
    FROM
      long_book_author_cte
  );
...
WITH
  decade_author_cte AS (
    SELECT
      FLOOR(published_date / 10) * 10 AS decade,
      author,
